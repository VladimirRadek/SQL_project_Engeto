--Table #1 with data for research on salaries, product prices over the years 
...
select 
	...
	cpri.price_unit 
from(
	--obtains salary data across industrues over the years
	select
		cpa.payroll_year,
		cpa.industry_branch_code,
		--AVG(cpa.value) to avoid duplicated data 
...
join (
	--obtains data about  of food and beverage prices broken down into categories over the years
	select 
		cpr.category_code,
